...
class StandClient(Services, BaseEvents, Generated):
    ...
    async def _read_loop(self):
        while self._running:
            ...
            if msg_leng <= 0:
                continue
            msg = await self._read_exactly(msg_leng, timeout=0.1)
            if not msg:
                continue
            if msg == b'\x01':
                self._pending_ping.set()
            else:
                asyncio.create_task(self._parse_and_handle(msg))

    async def _parse_and_handle(self, msg):
        parsed = await asyncio.to_thread(Parser.parse_response, msg)
        await self._handle_msg(parsed)
    ...
    async def _handle_msg(self, server_msg: ServerMsg):
        if server_msg.responses:
            tasks = []
            for response in server_msg.responses:
                fut = self._pending_requests.get(response.id)
                if fut:
                    fut.set_result(response)
                else:
                    pass
        if server_msg.events:
            event_tasks = [
                self._dp.call_listeners(
                    GENERATED_UPDATES[event.code][UPDATE_INDEX[event.code]](event.params.one)
                )
                for event in server_msg.events if GeneratedEvent.has_value(event.code)
            ]
            if event_tasks:
                await asyncio.gather(*event_tasks)
        if server_msg.compressed_instances:
            decompress_tasks = []
            for compressed in server_msg.compressed_instances:
                decompress_tasks.append(self._decompress_and_handle(compressed))
            if decompress_tasks:
                await asyncio.gather(*decompress_tasks)

    async def _decompress_and_handle(self, compressed):
        import lz4.block
        from .parser import Parser
        ...
        parsed = await asyncio.to_thread(Parser.parse_response, data)
        await self._handle_msg(parsed)
